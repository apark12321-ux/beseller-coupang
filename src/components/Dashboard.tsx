...
const STATUS_PILLS = ["all", "candidate", "needs_review", "excluded", "draft_saved", "register_failed"];
const PAGE_SIZE = 50;
...
export default function Dashboard() {
  ...
  const stages = [
    ...
    { no: "04", name: "등록(임시저장)", metric: counts.draft_saved || 0, sub: `/ ${counts.candidate || 0}`, bar: pct(counts.draft_saved || 0, counts.candidate || 0), cls: "ready", onClick: () => { setView("products"); setTab("draft_saved"); } },
  ];
  ...
  return (
    <div className="app">
      ...
      <div className="rail">
        {stages.map((s) => {
          const active = (view === "products" && ((s.no === "01" && tab === "all") || (s.no === "02" && tab === "candidate") || (s.no === "04" && tab === "draft_saved"))) || (view === "catmap" && s.no === "03");
          return (
